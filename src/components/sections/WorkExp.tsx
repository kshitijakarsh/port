        ...
        <section className="w-full p-4 sm:px-4 border-t-2 border-b-2 border-double">
            ...
            <div className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col gap-4">
                    {workExp.map((work) => (
                        <div key={work.id} className="group bg-muted/50 p-4 border border-border rounded-lg w-full transition-all duration-300">
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                                <div className="flex items-center gap-3">
                                    <h2 className="font-serif text-xl">{work.company}</h2>
                                    <WorkType worktype={work.type} />
                                </div>
                                <p className="text-xs text-muted-foreground whitespace-nowrap font-sans">{work.fromDate} - {work.toDate}</p>
                            </div>
                            <p className="text-sm text-muted-foreground mb-3">{work.title}</p>

                            <div className="flex flex-wrap gap-2 mb-4">
                                {work.techStack.map((tech) => (
                                    <span key={tech} className="font-sans text-[10px] bg-muted text-muted-foreground px-2 py-0.5 rounded-sm outline-1 outline-border outline-offset-1 transition-colors hover:text-foreground">
                                        {tech}
                                    </span>
                                ))}
                            </div>
...
export const WorkType = ({ worktype }: { worktype: string }) => {
    if (worktype === "Freelance") {
        return (
            <span className="text-xs outline-1 outline-amber-300 outline-offset-1 rounded-sm px-2 py-0.5 bg-amber-200/50 ">Freelance</span>
        )
    }
    if (worktype === "Internship") {
        return (
            <span className="text-xs outline-1 outline-blue-300 outline-offset-1 rounded-sm px-2 py-0.5 bg-blue-200/50">Internship</span>
        )
    }
    if (worktype === "Full Time") {
        return (
            <span className="text-xs outline-1 outline-green-300 outline-offset-1 rounded-sm px-2 py-0.5 bg-green-200/50">Full Time</span>
        )
    }
    return null;
}