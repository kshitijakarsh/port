  ...
  return (
    <section className="w-full px-4 sm:px-4 py-6">
          ...
          {blogs.map((blog) => (
            ...
            >
              <div className="flex flex-col gap-1">
                <p className="text-xl font-serif group-hover:text-foreground transition-colors">{blog.title}</p>
                <div className="flex gap-2">
                  {blog.tags.slice(0, 2).map((tag: any) => (
                    <span key={tag} className="text-xs text-muted-foreground bg-muted outline-1 outline-border outline-offset-1 px-1.5 py-0.5 rounded-sm">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="w-full flex justify-center pt-6">
        <Button variant="social" href="/blogs">See More</Button>
      </div>
    </section>
  );
}