        ...
        {isVisible && (
          <motion.div
            ...
            className={`absolute left-1/2 z-50 pointer-events-none ${
              isTop ? '-top-12' : 'top-full mt-2'
            }`}
          >
            <div className={`shadow-xl rounded-lg overflow-hidden ${className}`}>{content}</div>
            <div
              className={`absolute left-1/2 -translate-x-1/2 border-4 border-transparent ${
                isTop ? 'top-full border-t-black/20' : 'bottom-full border-b-black/20'
              }`}
            />
          </motion.div>
        )}
